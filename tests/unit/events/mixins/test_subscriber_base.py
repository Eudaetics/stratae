...
- subscribe (direct call) adds a handler to the mapping for a channel and returns the Handler.
- subscribe (decorator) adds a handler to the mapping for a channel and returns the Handler.
...
def test_subscribe_direct_call_adds_handler(subscriber_base: SubscriberBase[None]):
    ...
    # Act
    handle = subscriber_base.subscribe(channel, fn)

    # Assert
    assert handle in subscriber_base.get_handlers(channel)
...
def test_subscribe_adds_multiple_handlers_for_same_channel(subscriber_base: SubscriberBase[None]):
    ...
    # Act
    handle_a = subscriber_base.subscribe(channel, fn_a)
    handle_b = subscriber_base.subscribe(channel, fn_b)

    # Assert
    assert handle_a in subscriber_base.get_handlers(channel)
    assert handle_b in subscriber_base.get_handlers(channel)
...
def test_subscribe_registers_handlers_independently_per_channel(
    ...
    # Act
    handle_a = subscriber_base.subscribe(orders, fn_a)
    handle_b = subscriber_base.subscribe(shipments, fn_b)

    # Assert
    assert handle_a in subscriber_base.get_handlers(orders)
    assert handle_b not in subscriber_base.get_handlers(orders)
    assert handle_b in subscriber_base.get_handlers(shipments)
    assert handle_a not in subscriber_base.get_handlers(shipments)
...
def test_unsubscribe_removes_registered_handler(subscriber_base: SubscriberBase[None]):
    ...
    # Act
    subscriber_base.unsubscribe(channel, handle)

    # Assert
    assert handle not in subscriber_base.get_handlers(channel)
...
def test_unsubscribe_is_noop_when_handler_not_registered(subscriber_base: SubscriberBase[None]):
    ...
    channel = Channel("test")
    registered = Mock()
    handle = subscriber_base.subscribe(channel, registered)
    unregistered_handle = Handler(Mock())

    # Act & Assert
    subscriber_base.unsubscribe(channel, unregistered_handle)
    assert handle in subscriber_base.get_handlers(channel)
...
def test_subscribe_direct_call_stores_meta():
    ...
    Then: The returned Handler's meta attribute should be the provided meta instance
    """
    # Arrange
    ...
    # Act
    handle = base.subscribe(channel, fn, meta=meta)
...
def test_unsubscribe_is_noop_when_channel_has_no_handlers(subscriber_base: SubscriberBase[None]):
    ...
    # Act & Assert
    subscriber_base.unsubscribe(channel, Handler(Mock()))
