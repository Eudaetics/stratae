"""
Unit tests for the AsyncFactoryBoundEvent class.
...
from stratae.events import AsyncFactoryBoundEvent, Event, PubSub
...
def test_init_stores_event_emitter_and_config(
    ...
    When: An AsyncFactoryBoundEvent is created
    Then: The event, emitter, and config attributes should reference the supplied objects
    """
    emitter = create_autospec(_async_emit)
    config = object()

    bound = AsyncFactoryBoundEvent(
        emitter, payment_received, factory=_PaymentReceived, config=config
    )
...
def test_init_defaults_serializer_to_none(
    ...
    bound = AsyncFactoryBoundEvent(emitter, payment_received, factory=_PaymentReceived, config=None)
...
def test_init_stores_serializer(
    ...
    When: An AsyncFactoryBoundEvent is created with that serializer
    ...
    bound = AsyncFactoryBoundEvent(
        emitter, payment_received, factory=_PaymentReceived, config=None, serializer=serializer
    )
...
async def test_call_passes_positional_args_to_factory(
    ...
    Given: An AsyncFactoryBoundEvent whose factory accepts positional arguments
    When: The AsyncFactoryBoundEvent is called and awaited with positional arguments
...
async def test_call_passes_keyword_args_to_factory(
    ...
    Given: An AsyncFactoryBoundEvent whose factory accepts keyword arguments
    When: The AsyncFactoryBoundEvent is called and awaited with keyword arguments
    Then: The factory should be called with those values and the emitter
          should receive the constructed payload, the Event, and the config
    """
    spy = mocker.spy(_PaymentReceived, "__init__")
...
async def test_call_passes_mixed_args_to_factory(
    ...
    Given: An AsyncFactoryBoundEvent whose factory accepts positional and keyword args
    When: The AsyncFactoryBoundEvent is called and awaited with one positional and one keyword arg
...
async def test_call_returns_emitter_result(
    ...
    Given: An AsyncFactoryBoundEvent whose emitter resolves to a known value
    When: The AsyncFactoryBoundEvent is called and awaited
...
async def test_call_forwards_serializer_to_emitter(
    ...
    Given: An AsyncFactoryBoundEvent constructed with a serializer
    When: The AsyncFactoryBoundEvent is called and awaited
...
async def test_call_awaits_async_factory_before_passing_to_emitter() -> None:
    ...
    Given: An AsyncFactoryBoundEvent whose factory is a coroutine function
    When: The AsyncFactoryBoundEvent is called and awaited
    ...
    event = Event(_PaymentReceived, PubSub)
    emitter = create_autospec(_async_emit)
    bound = AsyncFactoryBoundEvent(emitter, event, factory=_async_factory, config=None)
