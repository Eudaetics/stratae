"""Test the msgspec integration's serialize registration for msgspec.Struct."""
...
class Point(msgspec.Struct):
    """A simple msgspec.Struct used to exercise the serialize registration."""

    x: int
    y: int


def test_serialize_returns_bytes_for_struct():
    """
    Test that serialize returns bytes for a msgspec.Struct.

    Given: A msgspec.Struct instance.
    When: The serialize function is called with the instance.
...
def test_serialize_struct_round_trips():
    """
    Test that serializing a msgspec.Struct produces its JSON representation.
...
def test_serialize_struct_uses_encode_for_unknown_field_types(mocker: MockerFixture):
    """
    Test that serialize wires encode in as msgspec's enc_hook for struct fields.

    Given: A msgspec.Struct field containing a value msgspec can't natively
        serialize.
    When: The serialize function is called with the struct.
    Then: encode is called with that value, and its return value is what
        ends up in the serialized output.
    """
...
def test_serialize_struct_raises_type_error_for_non_encodable_field():
    """
    Test that serialize raises TypeError for struct fields with no encode registration.
