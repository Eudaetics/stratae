...
def test_get_cache_not_in_stack(async_lifecycle: AsyncLifecycle, scopes: Sequence[str]):
    """
    ...
    with pytest.raises(RuntimeError, match=f"Scope '{scopes[2]}' is not active."):
        async_lifecycle.get_cache(scopes[2])
...
def test_push_invalid_scope(async_lifecycle: AsyncLifecycle, scopes: Sequence[str]):
    """
