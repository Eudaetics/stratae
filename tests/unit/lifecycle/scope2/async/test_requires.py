"""Unit tests for AsyncScope's `requires` parent relationship, async flavor."""
...
async def test_activate_without_active_parent_raises():
    """
    ...
    """
    # Arrange
    parent = AsyncScope("application", "shared")
    child = AsyncScope("request", requires=parent)
...
async def test_activate_with_active_parent_succeeds():
    """
    ...
    # Act & Assert
    async with parent.activate():
        async with child.activate():
            assert child.is_active()


async def test_deactivate_parent_while_child_active_raises():
    """
    ...
    with pytest.raises(
        ...
        await parent.deactivate(parent_activation)
    assert parent.is_active()


async def test_deactivate_parent_after_child_deactivated_succeeds():
    """
    ...
    await child.deactivate(child_activation)
...
async def test_multiple_children_of_same_parent():
    """
    ...
    # Arrange
    parent = AsyncScope("application", "shared")
    child_a = AsyncScope("session", requires=parent)
    child_b = AsyncScope("worker", requires=parent)
    parent_activation = parent.activate()
    child_a_activation = child_a.activate()
    child_b.activate()

    # Act
    await child_a.deactivate(child_a_activation)
...
async def test_three_level_chain_activates_and_deactivates_in_order(
...
async def test_three_level_chain_middle_scope_activate_requires_immediate_parent(
    async_scope_chain: tuple[AsyncScope, AsyncScope, AsyncScope],
):
    """
    ...
    When: the middle scope (session) is activated on its own, without application active,
    Then: activation still raises - a chain link only checks its own immediate parent, but
        that parent being inactive is enough to block it.
    """
    # Arrange
    _application, session, _request = async_scope_chain
...
async def test_three_level_chain_out_of_order_deactivation_blocked(
    async_scope_chain: tuple[AsyncScope, AsyncScope, AsyncScope],
):
    """
    ...
    # Act & Assert
    with pytest.raises(ScopeActivationError, match="a scope requiring it is still active"):
        await session.deactivate(session_activation)
    assert session.is_active()


@pytest.mark.parametrize("storage", ["dense", "sparse"])
async def test_deactivate_child_with_resource_cleans_up_exit_stack(storage: str):
    """
    Deactivating a scope with a required parent still runs its own exit stack cleanup.

    Given: A child scope requiring a parent, with a cached async resource entered during
        the child's activation,
    ...
    # Arrange
    parent = AsyncScope("application", "shared")
    child = AsyncScope("request", storage=storage, requires=parent)  # pyright: ignore[reportArgumentType]
    mock = Mock()
...
async def test_deactivate_raises_when_own_sparse_scope_has_active_dependent():
    """
    ...
    When: the parent is deactivated,
    Then: A ScopeActivationError is raised - `AsyncActivation`'s live-dependent-count
        guard catches this the same way for sparse storage as it does for dense.
    """
    # Arrange
    parent = AsyncScope("application", "shared", "sparse")
    child = AsyncScope("request", requires=parent)
    parent_activation = parent.activate()
    child.activate()
...
@pytest.mark.parametrize("storage", ["dense", "sparse"])
async def test_deactivate_child_without_resource_returns_none(storage: str):
    """
