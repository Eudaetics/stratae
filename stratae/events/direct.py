...
{py:func}`bind <stratae.events.binding.bind>` to get a callable that
...
`emit` method via {py:func}`abind <stratae.events.binding.abind>` to get an
...
from stratae.events.binding import AsyncBindMixin, BindMixin
...
class BaseDirectBus:
    ...
    inherit `bind` from
    {py:class}`BindMixin <stratae.events.binding.BindMixin>` or
    {py:class}`AsyncBindMixin <stratae.events.binding.AsyncBindMixin>`.
